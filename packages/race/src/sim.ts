...
/**
 * Физика кузова и колёс на Rapier. Здесь нет ни игры, ни правил: только тела,
 * подвеска, сцепление и коллизионная сетка дороги.
 *
 * Почему это вообще уместно при требовании воспроизводимости: у Rapier тот самый
 * контроллер машины, что и в Bullet, с рессорами и уводом шины, и он оказался
 * побитово детерминированным — два независимых мира с одинаковыми входами дали
 * одинаковые траектории до последнего знака. Цена — мегабайт WASM, поэтому пакет
 * подгружается динамически и только для этой игры.
 *
...
/** Загрузка WASM. Ядро без неё не шагает, поэтому её ждут до запуска игры. */
export async function prepareSim(): Promise<void> {
  if (api) return;
...
export function simReady(): boolean {
  return api !== null;
}
...
export { RIDE_HEIGHT_M, WHEEL_MOUNTS };
...
export interface SimControls {
  ...
  steer: number;
  /** Насколько машина вне асфальта: 0 — полоса, 1 — трава. */
  offroad: number;
}
...
export interface SimSave {
  ...
  steer: number;
  /**
   * Колёса теперь крутятся сами, а не следуют за кузовом, поэтому их вращение и
   * нагрев — часть состояния мира. Без них снимок терял бы буксование и сброшенное
...
export function createSim(seed: number, stamps: readonly ShapeStamp[], save?: SimSave): Sim {
  const R = api;
  if (!R) throw new Error("race: физика не загружена, нужен prepareSim()");
  ...
  const start = line.atDistance(save?.s ?? 0);
  /**
  ...
  const chassis = world.createRigidBody(
    R.RigidBodyDesc.dynamic()
      .setTranslation(start.x, start.y + RIDE_HEIGHT_M, start.z)
      .setRotation(yawQuat(start.h))
   ...
   */
  function applyTires(controls: SimControls): void {
    const q = chassis.rotation();
    const com = chassis.worldCom();
    const v = chassis.linvel();
    const w = chassis.angvel();
    const point = line.atDistance(s);
    ...
    for (let i = 0; i < 4; i++) {
      ...
      const ground = where ? surfaceAt(point, lateralOf(point, where.x, where.z)) : null;
      const surface: Surface = ground
        ? blendSurface(SURFACES[ground.kind]!, SURFACES[ground.next]!, ground.blend)
        : SURFACES.asphalt!;
      under[i] = ground ? (ground.blend > 0.5 ? ground.next : ground.kind) : "asphalt";
  ...
  function respawn(): void {
    const point = line.atDistance(s);
    const was = Math.abs(vehicle.currentVehicleSpeed());
    const speed = Math.min(was * 0.5, RESPAWN_SPEED_MAX);
    chassis.setTranslation({ x: point.x, y: point.y + RIDE_HEIGHT_M + 0.3, z: point.z }, true);
    chassis.setRotation(yawQuat(point.h), true);
    chassis.setLinvel({ x: Math.sin(point.h) * speed, y: 0, z: Math.cos(point.h) * speed }, true);
    ...
    for (let i = 0; i < 4; i++) omega[i] = speed / WHEEL_RADIUS_M;
  }
  ...
  return {
    step(dtS, controls) {
      ...
      while (accumulator >= PHYSICS_STEP_S && steps < MAX_STEPS_PER_TICK) {
        ...
        // Контроллер отрабатывает подвеску и заодно даёт нагрузку на колесо —
        // без неё шину считать не из чего, поэтому только потом шины.
    ...
    frame() {
      ...
      return {
        ...
        driveSpeedMs: driven,
        s,
        lateral: lateralOf(point, p.x, p.z),
        groundY: point.y,
        upright,
        wheels,
      };
